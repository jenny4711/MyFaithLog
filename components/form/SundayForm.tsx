...
const SundayForm = ({note,setNote,photo,setPhoto,showDone,lang,category}:any) => {
  ...
  return (
    <View style={{flex:1,width:width,justifyContent:'center',alignItems:'center'}}>
     ...
     <Animated.View style={[category==="source"?{display:'none'}:styles.inputView,{marginTop:10,marginBottom:10}]} exiting={FadeInLeft.duration(500).easing(Easing.ease)} entering={FadeInRight.duration(500).easing(Easing.ease)} >
  <TextInput
    style={{ width: width -96,height:320,backgroundColor:'white',padding:10,borderRadius:10}}
